# ganb_corporate_client.BulkTransferApi
...
api_instance = ganb_corporate_client.BulkTransferApi()
body = ganb_corporate_client.TransferCancelRequest() # TransferCancelRequest | HTTPリクエストボディ
...
# create an instance of the API class
api_instance = ganb_corporate_client.BulkTransferApi()
body = ganb_corporate_client.BulkTransferRequest() # BulkTransferRequest | HTTPリクエストボディ
x_access_token = 'x_access_token_example' # str | アクセストークン  minLength: 1 maxLength: 128 
...
request_transfer_class = '\"1\"' # str | 振込照会対象取得区分 半角数字 1：ALL、2：振込申請のみ、3：振込受付情報のみ 照会対象キー区分が2かつ、ビジネスID管理利用中の場合のときのみ入力可 それ以外はNULLを設定（値が設定されている場合は、「400 Bad Request」を返却） 照会対象キー区分が、2のときに指定しない場合は1と扱います  minLength: 1 maxLength: 1  (optional)
