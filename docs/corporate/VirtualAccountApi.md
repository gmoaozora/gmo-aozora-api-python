# ganb_corporate_client.VirtualAccountApi
...
api_instance = ganb_corporate_client.VirtualAccountApi()
x_access_token = 'x_access_token_example' # str | アクセストークン  minLength: 1 maxLength: 128 
va_contract_auth_key = '\"5CSDFG1515A54DF\"' # str | 振込入金口座API認証情報 半角英数字 銀行契約の方はNULLを設定 提携企業の方が、契約された顧客の入金明細照会を依頼される場合は必須 提携企業以外の方が値を設定されている場合は「400 Bad Request」を返却  minLength: 1 maxLength: 400  (optional)
ra_id = '\"101011234567\"' # str | 入金口座ID 半角数字 入金先の口座を識別するID 本値が設定されていない場合、振込入金口座IDは必須 科目コードが以下の口座IDのみ受け付けます ・01=普通預金（有利息） ・02=普通預金（決済用）  minLength: 12 maxLength: 29  (optional)
...
api_instance = ganb_corporate_client.VirtualAccountApi()
x_access_token = 'x_access_token_example' # str | アクセストークン  minLength: 1 maxLength: 128 
body = ganb_corporate_client.VaIssueRequest() # VaIssueRequest | HTTPリクエストボディ (optional)
...
api_instance = ganb_corporate_client.VirtualAccountApi()
x_access_token = 'x_access_token_example' # str | アクセストークン  minLength: 1 maxLength: 128 
body = ganb_corporate_client.VaStatusChangeRequest() # VaStatusChangeRequest | HTTPリクエストボディ (optional)
...
api_instance = ganb_corporate_client.VirtualAccountApi()
x_access_token = 'x_access_token_example' # str | アクセストークン  minLength: 1 maxLength: 128 
body = ganb_corporate_client.VaCloseRequest() # VaCloseRequest | HTTPリクエストボディ (optional)
...
api_instance = ganb_corporate_client.VirtualAccountApi()
x_access_token = 'x_access_token_example' # str | アクセストークン  minLength: 1 maxLength: 128 
body = ganb_corporate_client.VaListRequest() # VaListRequest | HTTPリクエストボディ (optional)
